...
ToDo:
\begin{itemize}
 \item Describe more clearly the approach
 \item Motivation for gamlss
 \item List of things still do
 \item Monthly plot
 \item Description of the elements in each plot
\end{itemize}
...
For the hindcast over the last 30 years we did not include the operational
...
is obtained from the Weerpluim (beter source?).
...
<<LoadData, echo = FALSE, cache = TRUE, dependson = 'LoadKIS'>>=
